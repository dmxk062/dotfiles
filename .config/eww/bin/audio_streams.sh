...
media_names="$(pactl list "$1" | grep 'media.name' | cut -d"=" -f 2 | sed -e 's/^\s*"//' -e 's/"\s*$//')"


pactl --format=json list "$1" 2>/dev/null | jq -cM --arg _names "$media_names" '. as $streams | $_names | split("\n") as $names |
