...
class EpubInformationPage(GObject.GObject, Nautilus.PropertiesModelProvider):
    def get_models(
        ...
        for val in metadata:
            section.append(Nautilus.PropertiesItem(name=val[1], value=val[0]))

        return [Nautilus.PropertiesModel(title=title, model=section)]
    ...
    def _get_meta_from_epub(self, path: str) -> list[tuple]:
        ...
        if date[0]:
            fdate = datetime.strftime(
                datetime.strptime(date[1], "%Y-%m-%dT%H:%M:%S%z"), "%Y/%m/%d"
            )
            entries.append((fdate, "Creation Date"))

        genres = [
            subject.text
            for subject in metadata.findall(".//dc:subject", namespaces=META_NS)
        ]
        if len(genres) > 0:
